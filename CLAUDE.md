...
Layered/DDD-style split with a strict one-way dependency: `engine` → `store`/`scene`/`ui`. The engine layer has zero React/Pixi imports and is 100% unit-testable in isolation.
...
- **`src/scene/`** — the 2D world drawn in code with Pixi.js v8 via `@pixi/react` (no image assets): `PixiCanvas.tsx` (root `Application`, world container following the boat), `riverPath.ts` (hand-authored vertical Catmull-Rom river path + waypoint positions), `vectorArt.ts` (all pure `Graphics` drawing functions: boat per tier/avatar, trees, houses, waste...), `palette.ts` (numeric colors), `camera.ts` (shared mutable camera/boat position, responsive world scale), `chunkCache.ts` (camera culling + `cacheAsTexture` for static chunks), plus layer components (`GroundTexture`, `RiverBackground`, `RiverMouthSea`, `RiverScenery`, `CharacterMarkers`, `WasteSprites`, `BoatSprite`, `CloudShadows`, `ScreenEffects`). `<pixiGraphics>` always needs a `draw` prop; per-frame animation mutates refs inside `useTick`.
- **`src/ui/`** — 2D React overlay components (HUD, quiz modal, waste-cleaning modal, start screen with the Mariana dam-disaster story and girl/boy avatar choice, game-over/victory screens) that read from the store and dispatch actions.
